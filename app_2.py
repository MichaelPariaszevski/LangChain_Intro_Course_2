...
from ice_breaker_final_2 import (
    ice_break_with_3,
...
@app_2.route("/")  # the home/index route
def index():
    return render_template("index.html")


@app_2.route("/process", methods=["POST"])
def process():
    name = request.form["name"]
    summary, profile_pic_url = ice_break_with_3(name=name)
    return jsonify(
        {
...
if __name__ == "__main__":
    app_2.run(host="0.0.0.0", debug=True)
